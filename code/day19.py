...
def part1():
    ...
    rules_parse = {}
    msgs = []
    for l in open('inputs/day19.txt', 'r').read().split('\n'):
        if ':' in l:
            p = l.split(':')
...
def part2():
    ...
    count = sum([bool(re.fullmatch(rules_done['0'].replace('x' , str(1)) , msg)) for msg in msgs])
    
    prv_count = 0
    rep = 2
    while prv_count != count:
        prv_count = count
        count += sum([bool(re.fullmatch(rules_done['0'].replace('x' , str(rep)) , msg)) for msg in msgs])
        rep +=1
