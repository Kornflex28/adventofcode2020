...
def valid_bags_rec(bag, bag_containers, valid_bags=set()):
    if bag not in bag_containers:
        ...
        for b in bag_containers[bag]:
            valid_bags = valid_bags.union(
                set(valid_bags_rec(b, bag_containers, valid_bags)))
...
if __name__ == "__main__":
    ...
    for rule in data:
        ...
        content = {bag: n for bag, n in list(map(lambda l: (" ".join(l[:-1].split(" ")[1:]).replace(
            " bags", "").replace(" bag", ""), int(l[:-1].split(" ")[0])), re.findall(r"\d+ \D+[,\.]+", rule)))}
